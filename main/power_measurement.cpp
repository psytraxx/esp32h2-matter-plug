...
// PowerMeasurementInit() below), and only this header declares
// GetClusterInstance(), which esp_matter's registration path requires.
#include "clusters/electrical_energy_measurement/integration.h"
 ...
 * (app_config.h) and must match the plug's own rating. */
...
public:
	 ...
	 * device actually measures. Declaring only ActivePower here leaves
	 * RMSVoltage and RMSCurrent published but undeclared, which is not
	 * conformant and gives a controller no accuracy/range to describe them
	 * with -- some controllers use this list to decide which sensors to
	 * surface at all.
	 *
	 * Ranges are the plug's plausible span, not measured figures; the
	 * percentages are placeholders pending the calibration bench work in
 ...
 * stack's own KeyValueStoreMgr(), which on ESP-IDF is backed by NVS -- no
 * separate storage handler is set up here.
 *
...
constexpr char kEnergyKvsKey[] = "cb2s-plug/cum-energy-mwh";
...
void PersistCumulativeEnergyIfDue(int64_t nowMs)
{
	...
	if (err != CHIP_NO_ERROR) {
		ESP_LOGE(TAG, "Failed to persist cumulative energy: %" CHIP_ERROR_FORMAT, err.Format());
		return;
	}
 ...
 *
 * So report on change, gated by a deadband -- these are far finer than the
 * hardware resolves (one CF pulse per second is on the order of a watt), so
 * in practice every genuine change reports and the deadband only suppresses
 * arithmetic jitter. */
...
CHIP_ERROR PowerMeasurementInit(EndpointId endpoint)
{
	 ...
	 * esp_matter has its own registration path instead:
	 * esp_matter::cluster::electrical_energy_measurement::create() is
	 * called on this endpoint in matter_setup.cpp's create_endpoints(),
	 * which registers an init callback
	 ...
	 * that constructs the real ElectricalEnergyMeasurementCluster once the
	 * data model has loaded -- by the time PowerMeasurementInit() runs
	 * (after esp_matter::start(), see matter_setup.cpp), that cluster
	 * already exists and GetClusterInstance()/SetMeasurementAccuracy()/
	 * NotifyCumulativeEnergyMeasured() (all free functions declared in
	 * CodegenIntegration.h) can be used directly. Feature bits (imported +
...
void PowerMeasurementUpdate(int64_t activePowerMw, int64_t rmsVoltageMv, int64_t rmsCurrentMa)
{
	...
	if (sHaveLastSample) {
		 ...
		 * controller will ever read. Rate-limit the rest: cumulative
		 * energy does not need the poll interval's resolution. */
		if (Server::GetInstance().GetFabricTable().FabricCount() > 0 &&
			...
			/* This esp_matter/CHIP snapshot's NotifyCumulativeEnergyMeasured()
			 * takes DataModel::Nullable<...>&, not chip::Optional<...> --
			 * differs from uascent-matter's nRF SDK snapshot, which took
			 * Optional. Use MakeNullable/NullNullable instead of
			 * MakeOptional/NullOptional. */
			ElectricalEnergyMeasurement::NotifyCumulativeEnergyMeasured(
				sEndpoint, DataModel::MakeNullable(imported), DataModel::NullNullable);
